...
# Note: while pie and donut charts are generally a bad idea for data viz because it's difficult to grasp the relative magnitude of each "slice", in this particular case I think it's alright because there are only two categories of sales to represent (tracks and albums) and I find that it actually conveys the relative proportions as well as or better than a bar chart. 
```
...
There are two ways we could define "most often" here - artists with the most tracks contained in playlists, and artists who have at least 1 track in the most playlists. Data for both definitions is gathered here:
...
```
1806 of the 3503 available tracks were purchased, or about 50%. This seems like a relatively low proportion, which may indicate that the range of tracks the store has available doesn't reflect their relative popularity as well as it could. This leads us to the next question:
...
pop_range <- ggplot(data = track_pop_range, aes(x = reorder(name, prop_sold), y = prop_sold, fill = name)) +
  geom_bar(stat = "identity") +
  coord_flip() +
  xlab("Genre") +
  ylab("Percent available tracks sold") +
  scale_y_continuous(labels = scales::percent) +
...
This makes sense, because only 3 such tracks were sold. This is only a toy example so I will leave it at that as this does not affect the general fit of the model per se (though it does make it less likely to perform well as a predictive model for new data). Otherwise it would be a good idea to try using an *exact* logistic regression, which works better than regular logistic regression when sample sizes are small and/or some cells have no observations. 

## Conclusion:
Our model indicates that media type does affect whether a track gets sold (protected media types get sold less), but the sub-types have very little effect on track popularity (the protected AAC type seems to increase the odds of a track being sold, but this may not be a reliable estimate given that the model is rank-deficient due to insufficient data). Non-protected media types are therefore more popular.
